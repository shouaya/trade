...
class SlippageModel {
   ...
   * @returns {boolean} - 是否为高滑点时段
   */
  isTokyoHighSlippageHour(kline) {
   ...
   * @param {boolean} isEntry - 是否为入场 (出场滑点通常更大)
   * @returns {number} - 总成本 (pips)
   */
   ...
   * @param {Object} kline - K线数据
   * @param {string} direction - 交易方向 ('long' / 'short')
   * @param {boolean} isEntry - 是否为入场
   * @returns {number} - 实际成交价格
   */
   ...
   * @param {boolean} isEntry - 是否为入场
   * @returns {Object} - 详细成本信息
   */
  getCostBreakdown(kline, direction, isEntry = true) {
    const signalPrice = parseFloat(kline.close);
    const volatility = this.calculateVolatility(kline);
