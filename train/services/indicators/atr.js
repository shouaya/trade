 ...
 * @param {Object} previous - 前一根K线
 * @returns {number} - True Range值
 */
...
function calculateATR(klines, period = 14) {
  ...
  const atrValues = Array(klines.length).fill(null);
  const trValues = [];
 ...
 * @param {Object} config - 配置 { slMultiplier, tpMultiplier }
 * @returns {Object} - { stopLoss, takeProfit }
 */
function calculateDynamicSLTP(atr, entryPrice, direction, config = {}) {
 ...
 * @param {number} riskPercent - 风险百分比 (例如: 0.01 = 1%)
 * @param {number} slMultiplier - 止损倍数
 * @returns {number} - 建议手数
 */
function calculatePositionSize(atr, accountBalance, riskPercent = 0.01, slMultiplier = 2.0) {
 ...
 */
function getATRAtIndex(klines, index, period = 14) {
  if (index < period) {
    return null;
  }
