 ...
 *
 * 使用方式:
 * const executor = new StrategyExecutorV3(strategy, klines, {
...
class StrategyExecutorV3 {
  constructor(strategy, klines, options = {}) {
    ...
    // V2功能配置
    this.enableMA200Filter = options.enableMA200Filter !== undefined
      ? options.enableMA200Filter
      : false;
    this.enableSlippage = options.enableSlippage !== undefined
      ? options.enableSlippage
      : false;

    // V3新功能配置
    this.enableMultiTimeframe = options.enableMultiTimeframe !== undefined
      ? options.enableMultiTimeframe
      : false;
    this.enableATRSizing = options.enableATRSizing !== undefined
      ? options.enableATRSizing
      : false;
    this.enableTrailingStop = options.enableTrailingStop !== undefined
      ? options.enableTrailingStop
      : false;
    this.enableRSIReversion = options.enableRSIReversion !== undefined
      ? options.enableRSIReversion
      : false;
    ...
    if (this.enableATRSizing || this.enableTrailingStop) {
      this.atrValues = ATRCalculator.calculateATR(this.klines, 14);
    }

    // 初始化多时间框架分析器 (如果启用)
    if (this.enableMultiTimeframe) {
      const rsiPeriod = this.strategy.parameters.rsi.period;
  ...
  calculateRSI() {
    ...
    const period = this.strategy.parameters.rsi.period;
    return RSICalculator.calculateRSI(closes, period);
  }
  ...
  getSignal(index) {
    ...
    const oversold = this.strategy.parameters.rsi.oversold;
    const overbought = this.strategy.parameters.rsi.overbought;
   ...
   */
  openPosition(kline, direction, index) {
    const params = this.strategy.parameters;

    // 获取成交价格 (如果启用滑点模型)
    const executionPrice = this.enableSlippage
    ...
    if (this.enableATRSizing) {
      ...
      if (atr) {
        ...
        lotSize = ATRCalculator.calculatePositionSize(
          atr,
          accountBalance,
          riskPercent,
          2.0  // SL = 2×ATR
    ...
    const position = {
      ...
      entry_index: index,
      entry_rsi: this.rsiValues[index],
      lot_size: lotSize,
      stop_loss: stopLoss,
   ...
   * 计算止损价格 (支持ATR动态止损)
   */
  calculateStopLoss(entryPrice, direction, risk, index) {
    // 如果启用ATR,使用ATR动态止损
    if (this.enableATRSizing) {
      const atr = this.atrValues[index];
      if (atr) {
   ...
   * 计算止盈价格 (支持ATR动态止盈)
   */
  calculateTakeProfit(entryPrice, direction, risk, index) {
    // 如果启用ATR,使用ATR动态止盈
    if (this.enableATRSizing) {
      const atr = this.atrValues[index];
      if (atr) {
  ...
  checkExitConditions(index, kline) {
    ...
    const currentPrice = this.enableSlippage
    ...
    // 1. 检查Trailing Stop
    if (this.enableTrailingStop && position.trailingActivated) {
    ...
    if (!shouldExit && position.stop_loss) {
   ...
   * 平仓
   */
  closePosition(kline, exitPrice, exitReason) {
    ...
    const trade = {
      ...
      pnl,
      hold_minutes: (parseInt(kline.open_time) - position.entry_time) / (1000 * 60)
    };
  ...
  async execute() {
    ...
    while (this.positions.length > 0) {
      ...
      const exitPrice = this.enableSlippage
        ? this.slippageModel.getExecutionPrice(lastKline, this.positions[0].direction, false)
...
module.exports = StrategyExecutorV3;
