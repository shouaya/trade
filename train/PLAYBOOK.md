...
- `train/reports/cost-sensitivity/`
- `train/reports/feature-causality/`
- `train/reports/regime-routing-results/`

### 脚本

- 正式运行入口放在 `train/src/scripts/`
- `train/scripts/` 只保留训练主链路仍直接调用的辅助脚本
- 能复用的逻辑优先抽到 `src/services/`
...
DB_HOST=127.0.0.1 node dist/scripts/router-validate.js \
  --validation configs/validation/<validation_config>.json \
