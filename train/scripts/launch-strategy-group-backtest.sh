...
echo "================================================================================"
echo "🚀 启动2025年策略分组并行回测"
echo "================================================================================"
echo ""
echo "配置:"
echo "  - 策略总数: 560,196"
echo "  - 并行进程: 10个 (每组~56,020个策略)"
echo "  - 时间范围: 2025年1-12月完整数据"
...
# 清理旧的结果表 (可选)
read -p "是否清理旧的结果表 backtest_results_2025_full? (y/N): " -n 1 -r
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
  echo "🗑️  清理旧的结果表..."
  mysql -h localhost -P 3306 -u root -prootpassword trading <<EOF
    DROP TABLE IF EXISTS backtest_results_2025_full;
EOF
  echo "✅ 清理完成"
  echo ""
fi
...
for group in 01 02 03 04 05 06 07 08 09 10; do
  GROUP_NUM=${group#0}
  SCRIPT="scripts/run-backtest-2025-group.js --group ${GROUP_NUM} --groups 10"
...
echo ""
echo "监控命令:"
echo "  查看所有进程: ps aux | grep 'run-backtest-2025-group'"
