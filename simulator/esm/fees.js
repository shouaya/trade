...
export function findSettlementMarkPrice(klines, startIndex, endIndex, settlementTime, fallbackPrice) {
...
export function calculateCommissionFee(params) {
    const { position, exitPrice, exitTime, exitIndex, feeModel, symbolSpec, klines, activationIndex, } = params;
    ...
    for (const settlementTime of settlementTimes) {
        const markPrice = findSettlementMarkPrice(klines, activationIndex ?? position.entryIndex, exitIndex, settlementTime, exitPrice);
...
export function calculateTradeOutcome(params) {
    ...
    const commissionFee = calculateCommissionFee(activationIndex === undefined
        ? {
            position,
            exitPrice,
            exitTime,
