...
// 2026年可用数据范围 (1-2月,含60天预热期)
const WARMUP_DAYS = 60;
const YEAR_START = '2026-01-01T00:00:00Z';
const YEAR_END = '2026-02-28T23:59:59Z';  // 只有1-2月数据
...
async function main() {
  console.log('\n' + '='.repeat(80));
  console.log('🔍 2026年策略验证 (1-2月数据)');
  console.log('   验证2025年Top 3策略在2026年1-2月的表现');
  console.log('='.repeat(80) + '\n');

  const overallStartTime = Date.now();

  try {
    // 1. 加载2026年完整K线数据 (含60天预热期)
    console.log('📊 加载2026年1-2月K线数据 (含60天预热期)...\n');
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      const result2026 = results[i];

      console.log('策略', i + 1 + ':', result2026.name, '\n');

      console.log('  指标对比:');
      const tradeChange = result2025.trades > 0 ? ((result2026.stats.totalTrades - result2025.trades) / result2025.trades * 100).toFixed(1) : 'N/A';
      const winRateChange = (result2026.stats.winRate * 100 - result2025.winRate).toFixed(2);
      const pnlChange = result2025.pnl !== 0 ? ((result2026.stats.totalPnl - result2025.pnl) / result2025.pnl * 100).toFixed(1) : 'N/A';
      const scoreChange = result2025.score !== 0 ? (((result2026.stats.score || 0) - result2025.score) / result2025.score * 100).toFixed(1) : 'N/A';

      console.log('    交易次数:', result2025.trades, '(2025) →', result2026.stats.totalTrades, '(2026) [' + (result2026.stats.totalTrades > result2025.trades ? '↑' : '↓'), tradeChange + '%]');
      console.log('    胜率:', result2025.winRate.toFixed(2) + '% (2025) →', (result2026.stats.winRate * 100).toFixed(2) + '% (2026) [' + (result2026.stats.winRate * 100 > result2025.winRate ? '↑' : '↓'), winRateChange + '%]');
      console.log('    总盈亏: $' + result2025.pnl, '(2025) → $' + result2026.stats.totalPnl.toFixed(2), '(2026) [' + (result2026.stats.totalPnl > result2025.pnl ? '↑' : '↓'), pnlChange + '%]');
      console.log('    综合评分:', result2025.score, '(2025) →', (result2026.stats.score || 0).toFixed(2), '(2026) [' + ((result2026.stats.score || 0) > result2025.score ? '↑' : '↓'), scoreChange + '%]');
      console.log('');

      // 一致性评估
      const isConsistent =
        result2026.stats.totalPnl > 0 &&
        result2026.stats.winRate > 0.48 &&
        result2026.stats.profitFactor > 1.0;
    ...
    if (allProfitable && allPositiveWinRate && allPositiveProfitFactor) {
      console.log('  🎉 结论: Top 3策略在2026年表现一致,可以考虑实盘测试!');
    } else {
      console.log('  ⚠️  结论: 部分策略在2026年表现不佳,需要进一步分析或优化。');
    }
    console.log('');
